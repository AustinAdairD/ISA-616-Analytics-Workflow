...
In this paper I attempted to discern what factors contribute to sales price of a home in Ames Iowa. The data analyzed contains housing sales prices from 2006 through 2010. Access to the database can be gained from the Ames site (http://www.cityofames.org/assessor/) by clicking on “property search” or by accessing the Beacon (http://beacon.schneidercorp.com/Default.aspx) website and inputting Iowa and Ames in the appropriate fields.

The primary purpose for this paper is to provide guidance for - Friedrich Land Development; who has been engaged in real-estate development in Ames Iowa for over 50 years. Specifically, this analysis will give insights into what housing features have the highest correlation to final sales price of a house. This will allow them to better evaluate the true value of houses currently on the market and understand what features should be added to generate the highest ROI. 
...
Now that we understand our data we can clean it by removing variables that are missing more than 45% of their values.next we will impute the impute the mean of numeric values and the mode of categorical values that are missing least 5% of their values. In order to  Once we have fixed the majority of the missing values we can then omit any variable that is still missing. In doing so we will only be eliminating 104 observation, which should have a minimal impact on developing a model.  
...
In conclusion, our model was successful in answering the original question of what housing features are most important to predicting the sales price of a home. Although our final model contained a plethora of statistically significant variables there were some that had exceptionally high correlations. 

In analyzing our model we can conclude that the most important factor in predicting the final sale price of a house would be the roofing materiel used. In a business sense, this makes sense, we can extrapolate that more expensive houses would likely have more expensive roofing material. This, information can help guide Friedrich Land Development determine what roofing material they can utilize for the best ROI. 
